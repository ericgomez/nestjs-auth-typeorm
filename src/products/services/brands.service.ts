...
export class BrandsService {
  constructor(@InjectRepository(Brand) private brandsRepo: Repository<Brand>) {} // 👈 Inject
  ...
  findOne(id: number) {
    const product = this.brandsRepo.findOne({
      relations: ['products'], // implementamos la relacion con productos que esta dentro de la entidad Brand
    });
    if (!product) {
